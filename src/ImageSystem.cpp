...
void ImageSystem::addTopic(const std::string& TopicName) {
    ...
    GLCALL(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, 10, 10, 0, GL_RGB, GL_UNSIGNED_BYTE, NULL));
    ...
    m_ImageProcessors.at(current_size).image.Width = 0;
    m_ImageProcessors.at(current_size).image.Height = 00;
}
    ...
    for (int i = 0; i < m_ImageProcessors.size(); i++)
    {
        ...
        if (m_ImageProcessors.at(i).dirty)
        {
            ...
            if (m_ImageProcessors.at(i).texture_width != m_ImageProcessors.at(i).image.Width || m_ImageProcessors.at(i).texture_height != m_ImageProcessors.at(i).image.Height)
            {
                ...
                GLCALL(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, m_ImageProcessors.at(i).texture_width, m_ImageProcessors.at(i).texture_height, 0, GL_RGB, GL_UNSIGNED_BYTE, m_ImageProcessors.at(i).image.Data.data()));                
            }
            else
            {
        ...
        if (ImGui::Begin(m_ImageProcessors.at(i).topic_name.c_str()))
        {
            ...
            auto image_aspect = static_cast<float>(m_ImageProcessors.at(i).image.Width) / static_cast<float>(m_ImageProcessors.at(i).image.Height);
            ...
            ImGui::Image(static_cast<ImTextureID>(m_ImageProcessors.at(i).texture), display_size);
        }
        ImGui::End();
    }
