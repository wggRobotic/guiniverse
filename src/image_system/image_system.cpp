...
void ImageSystemImage::imgui_image(bool flip_vertically, bool flip_horizontally)
{
...

    {
        ...
        {
            GLCALL(glBindTexture(GL_TEXTURE_2D, gl_texture));
            if (image.cols != texture_width || image.rows != texture_height)
            {
    ...
    if (!empty)
    {
        ...
        ImGui::Image((ImTextureID) gl_texture, display_size, ImVec2(flip_horizontally ? 1.f : 0.f, flip_vertically ? 1.f : 0.f), ImVec2(flip_horizontally ? 0.f : 1.f, flip_vertically ? 0.f : 1.f));
    }
    else
    {
...
void ImageSystem::AddOnThreadFunction(int index)
{
    while (!m_ImageProcessors[index]->addons.thread_should_close.load())
    {
            ...
            && !image.empty())
        {
            ...
            quirc_resize(
                m_ImageProcessors[index]->addons.qrcode.quirc_instance,
                image.cols,
                image.rows);
...
void ImageSystem::onGuiFrame()
{
    for (size_t i = 0; i < m_ImageProcessors.size(); i++)
    {
        ...
        if (ImGui::Begin(m_ImageProcessors[i]->imgui_panel_name.c_str()))
        {
            ...
            m_ImageProcessors[i]->image.imgui_image(
                m_ImageProcessors[i]->flip_vertically,
                m_ImageProcessors[i]->flip_horizontally);
        }
        ImGui::End();
        ...
        {
            if (ImGui::Begin(
                    (m_ImageProcessors[i]->imgui_panel_name + " - diff").c_str()))
            {
                m_ImageProcessors[i]->addons.diff.image.imgui_image(
                    m_ImageProcessors[i]->flip_vertically,
                    m_ImageProcessors[i]->flip_horizontally);
            }
            ImGui::End();
        ...
        {
            if (ImGui::Begin(
                    (m_ImageProcessors[i]->imgui_panel_name + " - grayscale").c_str()))
            {
                m_ImageProcessors[i]->addons.grayscale.image.imgui_image(
                    m_ImageProcessors[i]->flip_vertically,
                    m_ImageProcessors[i]->flip_horizontally);
            }
            ImGui::End();
