...
DataCaptureSystem::DataCaptureSystem(std::shared_ptr<rclcpp::Node> Node)
    : m_Node(Node)
{
    m_Sections.reserve(3);
}

#define MAKE_DATA_CAPURE_SYSTEM_CALLBACK(INDEX)               \
    [this, INDEX](const std_msgs::msg::String::SharedPtr msg) \
    {                                                         \
        Callback(INDEX, msg);                                 \
    }
...
void DataCaptureSystem::ImGuiPanels()
{
    ...
    for (int i = 0; i < m_Sections.size(); i++)
    {
        ...
        {
            for (const auto& [code, has_code] : m_Sections.at(i).Data)
                ImGui::Text("%s: %lu", code.c_str(), has_code);
