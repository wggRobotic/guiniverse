    ...
    if (m_RefreshButton)
    {
        ...
        for (int i = 0; i <= GLFW_JOYSTICK_LAST; i++)
        {
            ...
            if ((m_InputDevicesConnected[i] = glfwJoystickPresent(GLFW_JOYSTICK_1 + i)) != 0)
            {
                ...
                if (name)
                    m_InputDeviceNames[i].assign(name);
                else
                    m_InputDeviceNames[i].assign("Name_Error");
            }
            else if (i == m_InputDeviceSelected)
...
void JoystickInput::ImGuiPanel(char* label)
{
    if (ImGui::Begin(label))
    {
        if (ImGui::BeginCombo(
                ...
                m_InputDeviceSelected == -1
                    ? "No device selected"
                    : m_InputDeviceNames[m_InputDeviceSelected].c_str()))
        {
            if (ImGui::Selectable("No device selected", m_InputDeviceSelected == -1))
                m_InputDeviceSelected = -1;

            for (int i = 0; i <= GLFW_JOYSTICK_LAST; i++)
                if (m_InputDevicesConnected[i] == true)
                {
