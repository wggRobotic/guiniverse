...
{
    {
        ...
        if (key_w || key_a || key_s || key_d)
        {
            ...
            if (key_w)
                m_Input.drive.main_axis_y = 1.f;
            if (key_a)
                m_Input.drive.main_axis_x = 1.f;
            if (key_s)
                m_Input.drive.main_axis_y = -1.f;
            if (key_d)
                m_Input.drive.main_axis_x = -1.f;
        ...
        float length = sqrtf(
            m_Input.drive.main_axis_x * m_Input.drive.main_axis_x
            + m_Input.drive.main_axis_y * m_Input.drive.main_axis_y);
        if (length > 1.0f)
        {
        ...
        if (glfwGetKey(window, GLFW_KEY_J) == GLFW_PRESS)
            m_Input.gripper.forward_axis = -1.f;
        if (glfwGetKey(window, GLFW_KEY_U) == GLFW_PRESS)
            m_Input.gripper.forward_axis = 1.f;
        if (glfwGetKey(window, GLFW_KEY_K) == GLFW_PRESS)
            m_Input.gripper.up_axis = -1.f;
        if (glfwGetKey(window, GLFW_KEY_I) == GLFW_PRESS)
            m_Input.gripper.up_axis = 1.f;
        if (glfwGetKey(window, GLFW_KEY_L) == GLFW_PRESS)
            m_Input.gripper.ground_angle_axis = -1.f;
    ...
    if (ImGui::Begin("Control"))
    {
        ...
        if (ImGui::IsItemActive())
            m_Input.drive.enable_button_physical = true;
        ImGui::Button("Disable");
        if (ImGui::IsItemActive())
            m_Input.drive.disable_button_physical = true;
        ...
        ImVec2 drive_joystick_axes = ImVec2(
            m_Input.drive.main_axis_x,
            m_Input.drive.main_axis_y);
        ...
        drive_joystick_axes = imgui_joystick(
            ...
            (m_Input.drive.gas_button ? IM_COL32(150, 150, 150, 255) : (80, 80, 80, 255)));
        ...
        ImGui::VSliderFloat(
            "##drive_slider_scalar",
            ImVec2(20, 200),
            &m_Input.drive.scalar_axis,
            0.0f,
            1.0f,
            "%.2f");

        ImGui::SetCursorScreenPos(ImVec2(pos.x + 400.f, pos.y));
        m_Input.gripper.forward_axis = imgui_joyslider(
            "gripper_slider_forward",
            200.f,
            0.05f,
            (m_Input.gripper.forward_axis == 0.f ? 0 : &m_Input.gripper.forward_axis));

        ImGui::SetCursorScreenPos(ImVec2(pos.x + 450.f, pos.y));
        m_Input.gripper.up_axis = imgui_joyslider(
            "gripper_slider_up",
            200.f,
            0.05f,
            (m_Input.gripper.up_axis == 0.f ? 0 : &m_Input.gripper.up_axis));

        ImGui::SetCursorScreenPos(ImVec2(pos.x + 500.f, pos.y));
        m_Input.gripper.ground_angle_axis = imgui_joyslider(
            "gripper_slider_ground_angle",
            200.f,
            0.05f,
            (m_Input.gripper.ground_angle_axis == 0.f ? 0 : &m_Input.gripper.ground_angle_axis));

        ImGui::SetCursorScreenPos(ImVec2(pos.x + 550.f, pos.y));
        ImGui::VSliderFloat(
            "##gripper_slider_gripper",
            ImVec2(20, 200),
            &m_Input.gripper.gripper_state,
            0.0f,
            1.0f,
            "%.2f");
    }
    ...
    if (ImGui::Begin("WheelVisualization"))
    {
        ...
        for (int i = 0; i < m_Wheels.size(); i++)
        {
            imgui_arrow(
                ...
                m_Wheels.at(i).target_angle,
                m_Wheels.at(i).target_rpm * (m_Wheels.at(i).invert ? -1.f : 1.f),
            ...
            imgui_arrow(
                ImVec2(-m_Wheels.at(i).y * 500.f + 150.f, -m_Wheels.at(i).x * 500.f + 200.f),
                m_Wheels.at(i).last_angle,
                m_Wheels.at(i).last_rpm * (m_Wheels.at(i).invert ? -1.f : 1.f),
...
#define chasis_line(x_0, y_0, x_1, y_1) \
    imgui_line(ImVec2(offset.x + (x_0) * scalar, offset.y - (y_0) * scalar), ImVec2(offset.x + (x_1) * scalar, offset.y - (y_1) * scalar), IM_COL32(150, 150, 150, 255), 3.f)
        ...
        imgui_arrow(
            ImVec2(
                (m_Gripper.target_x + chassis.bottom - chassis.gripper_x) * scalar
                    + offset.x,
                offset.y - (m_Gripper.target_y + chassis.gripper_y) * scalar),
            m_Gripper.target_ground_angle - M_PIf / 2.f,
            20.f,
