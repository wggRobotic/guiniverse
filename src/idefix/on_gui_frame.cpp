...
{
    {
        ...
        if (glfwGetKey(window, GLFW_KEY_W))
            m_Input.main_axes.y = 1.f;
        else if (glfwGetKey(window, GLFW_KEY_S))
            m_Input.main_axes.y = -1.f;

        float length = sqrtf(
            m_Input.main_axes.x * m_Input.main_axes.x
            + m_Input.main_axes.y * m_Input.main_axes.y);
        if (length > 1.0f)
            m_Input.main_axes = ImVec2(
                m_Input.main_axes.x / length,
                m_Input.main_axes.y / length);
    ...
    if (ImGui::Begin("Control"))
    {
        ...
        m_Input.main_axes = imgui_joystick(
            "virtual joystick",
            200.f,
            ImVec2(0.2f, 0.2f),
        ...
        ImGui::SetCursorScreenPos(
            ImVec2(pos.x + 260.f, pos.y + ImGui::GetStyle().ItemSpacing.y));

        ImGui::VSliderFloat("##slider_roll", ImVec2(20, 200), &m_Input.roll, -1.0f, 1.0f, "%.2f");

        ImGui::SetCursorScreenPos(
            ImVec2(pos.x + 290.f, pos.y + ImGui::GetStyle().ItemSpacing.y));

        ImGui::VSliderFloat("##slider_pitch", ImVec2(20, 200), &m_Input.pitch, -1.0f, 1.0f, "%.2f");

        ImGui::SetCursorScreenPos(
            ImVec2(pos.x + 320.f, pos.y + ImGui::GetStyle().ItemSpacing.y));

        ImGui::VSliderFloat("##slider_yaw", ImVec2(20, 200), &m_Input.yaw, -1.0f, 1.0f, "%.2f");

        ImGui::SetCursorScreenPos(
            ImVec2(pos.x + 350.f, pos.y + ImGui::GetStyle().ItemSpacing.y));
