...
ImVec2 imgui_joystick(const char* label, float size, ImVec2 dead_ranges, ImVec2* position, ImU32 color)
{
    ...
    else if (ImGui::IsItemActive())
    {
        ...
        joystick_position = ImVec2(0.f, 0.f);

    float length = sqrtf(
        joystick_position.x * joystick_position.x
    ...
    if (fabsf(joystick_position.x) < dead_ranges.x)
        joystick_position.x = 0.0f;
    if (fabsf(joystick_position.y) < dead_ranges.y)
        joystick_position.y = 0.0f;
    ...
    ImGui::SetCursorScreenPos(
        ImVec2(cursorPos.x, cursorPos.y + size + ImGui::GetStyle().ItemSpacing.y));
...
float imgui_joyslider(const char* label, float size, float dead_range, float* position)
{
    ...
    if (fabsf(joyslider_position) < dead_range)
        joyslider_position = 0.0f;
    ...
    draw_list->AddCircleFilled(ImVec2(handle_mid.x, handle_mid.y - joyslider_position * 0.5f * size), size * 0.1f, IM_COL32(255, 0, 0, 255), 32);
...
void imgui_arrow(ImVec2 start, float angle, float magnitude, ImU32 color, float thickness, float arrowSize, bool showAmplitude)
{
    ...
    ImVec2 arrowLeft = ImVec2(end.x + dir.x * arrowSize * (magnitude >= 0 ? -1.f : 1.f) + perp.x * (arrowSize * 0.5f), end.y + dir.y * arrowSize * (magnitude >= 0 ? -1.f : 1.f) + perp.y * (arrowSize * 0.5f));
    ImVec2 arrowRight = ImVec2(end.x + dir.x * arrowSize * (magnitude >= 0 ? -1.f : 1.f) - perp.x * (arrowSize * 0.5f), end.y + dir.y * arrowSize * (magnitude >= 0 ? -1.f : 1.f) - perp.y * (arrowSize * 0.5f));
